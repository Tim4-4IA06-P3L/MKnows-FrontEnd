...
const Page = () => {
  ...
  return (
		<main className="relative">
			...
			<aside className={`fixed top-[15%] left-0 bg-neutral-100 p-8 h-[80%] w-[300px] rounded-br-lg shadow-lg 
			...
			>
				...
				>
					<ul className="text-black list-disc list-inside leading-loose">
						{categories.map((category) => (
							category.programs.length > 0 &&
							<li key={category.id}><a href={`#${category.id}`} onClick={() => setToggle(0)} className="hover:underline">{category.Category}</a></li>
						))}
					</ul>
				</div>
			</aside>
